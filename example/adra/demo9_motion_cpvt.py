...
def print_help():
    ...
    print("./demo9_motion_cpvt arg1 arg2")
...
def main():
    ...
    run command(USB-To-RS485 + COM:/dev/ttyUSB0):
        python3 example/adra/demo9_motion_cpvt.py 1 0
    run command(EtherNet-To-RS485 + IP:192.168.1.16):
        python3 example/adra/demo9_motion_cpvt.py 3 16
    """
    ...
    id = [1, 2, 3]
    vels = [100, -60, 20]
    filter_param = [999, 100, 1]

    for i in range(number):
        adra.connect_to_id(id[i])  # Step 1: Connect an actuator

        ret = adra.into_motion_mode_vel()  # Step 2: Set the motion mode to velocity mode.
        ...
        ret = adra.set_vel_output_filter_param(filter_param[i])  # Step 5: Set the filter param.
        check_ret(ret, "set_vel_output_filter_param")

        ret = adra.set_vel_target(vels[i])  # Step 6: Set the target velocity.
        check_ret(ret, "set_vel_target")

    time.sleep(1)

    while(1):
        rets, broadcast_num, pos, vel, tau = adra.get_cpvt_current(id[0], id[number - 1])
        print("get_cpvt_current: pos: %f %f %f, vel: %f %f %f, tau: %f %f %f" %
              (pos[0], pos[1], pos[2], vel[0], vel[1], vel[2], tau[0], tau[1], tau[2]))
        print("get_cpvt_current: broadcast_num:%d %d %d, ret:%d %d %d\n" %
